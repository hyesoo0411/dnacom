...
from utils.quant import ternary_quantize
...
def train_step(
    ...
    for batch, (X, y) in enumerate(dataloader):
            ...
            with torch.no_grad():  # Apply quantization for forward pass only
                ...
                for name, param in model.named_parameters():
                    if "weight" in name:
                        ...
                        scale = calculate_scale(param.data, scale_factor=scale_factor)
                        scales[name] = scale
                        param.data = ternary_quantize(param.data, scale)
        ...
        y_pred = apply_noise(model.classifier(linear_output), noise_range=(1-model.noise, 1+model.noise))  # Pass through the classifier
        # y_pred = model(X)
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            if apply_ternary_quantization:
                with torch.no_grad():  # Apply quantization for forward pass only
                    ...
                    for name, param in model.named_parameters():
                        if "weight" in name:
                            scale = calculate_scale(param.data, scale_factor=scale_factor)  # Calculate scale
                            scales[name] = scale
                            original_weights[name] = param.data.clone()
                            param.data = ternary_quantize(param.data, scale)  # Apply ternary quantization
