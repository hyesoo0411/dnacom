...
def ternary_quantize(weights, threshold=None, percentage=0.05):
    ...
    if threshold is None:
        ...
        threshold = dynamic_threshold(weights, percentage)
...
def train_qat(
    ...
    for epoch in range(epochs):
        ...
        test_loss, test_acc = test_step(model=model, dataloader=test_dataloader, loss_fn=loss_fn, device=device, apply_ternary_quantization=apply_ternary_quantization)
        # Print out what's happening
        print(
            f"Epoch: {epoch+1} | "
            f"train_loss: {train_loss:.4f} | "
