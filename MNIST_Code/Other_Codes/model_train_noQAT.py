"""
model_train_noQAT.py: Pruning => 가중치만 -1 0 1로 양자화 
"""
...
import data_setup, engine_noQAT, model_builder, utils
...
NUM_EPOCHS = 20
BATCH_SIZE = 32
HIDDEN_UNITS = 10  # 200 => 10(pruned), 400 => 20, 800 => 40, 1600 => 80
LEARNING_RATE = 0.001  # 0.001, 0.0005, 0.0001, 0.00005
apply_ternary_quantization = False
bias = False
# pruning = True

# Setup directories
train_dir = "/home/hyesoo/MNIST/Dataset/train"
test_dir = "/home/hyesoo/MNIST/Dataset/test"
...
results = engine_noQAT.train_qat(
    model=model,
...
utils.save_model(
    model=model,
    target_dir="/home/hyesoo/MNIST/models",
    model_name=f"WSmodel_{HIDDEN_UNITS}units_lr{LEARNING_RATE}_epoch{NUM_EPOCHS}_bias{bias}.pth",
)
...
#             non_zero_connections = torch.sum(classifier_weight != 0, dim=0).bool()  # 0이 아닌 연결 찾기 (입력 유닛 기준)
#             print(f"Classifier {name} - Removed {classifier_weight.size(1) - non_zero_connections.sum().item()} input connections")
...
#             print(f"Pruned classifier: {name} - Removed {classifier_weight.size(1) - pruned_weight.size(1)} input connections")
#         else:
...
def plot_loss_curves(results: Dict[str, List[float]]):
    ...
    plt.legend()
    plt.savefig(
        f"/home/hyesoo/MNIST/LossCurve/LossCurve_{HIDDEN_UNITS}units_lr{LEARNING_RATE}_epoch{NUM_EPOCHS}_Pruning_wQAT-101{apply_ternary_quantization}_bias{bias}.png"
    )
