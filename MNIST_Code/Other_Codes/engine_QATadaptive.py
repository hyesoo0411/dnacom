...
def adaptive_ternary_quantize(weights, a, threshold=None):  # threshold: 0.05, 0.01, 0.1
    """
    Applies ternary quantization to the given weights.
    Maps values to {-a, 0, a}.
    ...
    if threshold is None:
        # 최대 절대값의 5%를 임계값으로 설정
        threshold = weights.abs().max() * 0.05  # 0.05는 바뀔 수 있음 (실험으로 적절한 값 설정 필요)

    quantized_weights = torch.zeros_like(weights)  # Initialize tensor of zeros with same shape
    quantized_weights[weights > threshold] = a  # Values above threshold become 1
    quantized_weights[weights < -threshold] = -a  # Values below negative threshold become -1
...
def train_step(
    ...
    apply_adaptive_ternary_quantization: bool = True,
) -> Tuple[float, float]:
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        if apply_adaptive_ternary_quantization:
            with torch.no_grad():
                for name, param in model.named_parameters():
                    if "weight" in name:  # Apply to weights only
                        param.data = adaptive_ternary_quantize(param.data, a)
