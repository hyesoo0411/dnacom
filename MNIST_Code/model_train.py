...
from configurator import resolve_config_from_cli
import os

# Load configurations
config = resolve_config_from_cli()
...
# # 2. Parse remaining CLI arguments as overrides
# cli_overrides = OmegaConf.from_dotlist(sys.argv[2:])

# # 3. Merge the CLI overrides into the base config
...
utils.save_model(
    ...
    model_name=f"WSmodel_{config.hidden_units}units_lr{config.learning_rate}_epoch{config.num_epochs}_wQAT5{config.apply_quantization}_round_bias{config.bias}_noise{config.noise}.pth",
)
...
def plot_loss_curves(results: Dict[str, List[float]]):
    ...
    os.makedirs(config.loss_curve_dir, exist_ok=True)

    plt.savefig(
        f"{config.loss_curve_dir}/LossCurve_{config.hidden_units}units_lr{config.learning_rate}_epoch{config.num_epochs}_wQAT5{config.apply_quantization}_round_bias{config.bias}_noise{config.noise}.png"
    )
    print(f"Loss curves saved to {config.loss_curve_dir}/LossCurve_{config.hidden_units}units_lr{config.learning_rate}_epoch{config.num_epochs}_wQAT5{config.apply_quantization}_round_bias{config.bias}_noise{config.noise}.png")
